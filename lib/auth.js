...
export function createUser({ name, email, password, role }) {
  ...
  run(
    `INSERT INTO users (id, role, name, email, password_hash, password_salt)
     VALUES ($id, $role, $name, $email, $hash, $salt)`,
    { $id: id, $role: role, $name: name, $email: email.toLowerCase(), $hash: hash, $salt: salt }
  );
  return id;
}

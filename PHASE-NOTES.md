...
## Phase 13 — Verified reference checks between hosts

- **The problem this solves**: a rental application's "previous landlord" field was always self-reported by the applicant — a name and phone number they typed in themselves, unverifiable by the platform. This phase adds a second, *verified* path alongside it: an applicant's own completed booking/lease history on Kanto, which can't be fabricated because it's a real record tied to a real past host.
- **`reference_requests` and `reference_messages` tables** (`lib/db.js`) — a reference request links a rental application, the applicant, the reviewing ("requesting") host, a past host, and the specific completed booking that proves the relationship. Its own private message thread (`reference_messages`) works exactly like the existing booking/application/inquiry messaging — 2-party only, this time between the requesting host and the past host. The applicant is never a participant and can't read the thread, only see that a request exists and its status (enforced server-side: `handleReferenceDetail` in `server.js` 403s anyone but those two hosts).
- **On a rental application's detail page** (`views/application.js`), once the applicant has consented (the existing consent checkbox on the application form was reworded to cover this — see below), the reviewing host sees a "Verified previous landlords on Kanto" panel: up to 3 of the applicant's most recent *completed* stays elsewhere on Kanto, deduped by host, excluding the reviewing host themself. Each row gets a "Request reference" button — clicking it re-verifies server-side that the booking is real (never trusts the posted host/booking id outright) and opens a new private thread with that past host at `/references/:id`.
- **The reference thread page** (`views/reference.js`) gives the past host a simple free-text reply plus three one-click buttons ("Would rent to again" / "Neutral" / "Would not rent again") that set a `rating` on the request and mark it `responded`. The requesting host sees the conversation and the rating; nothing here is a real background/criminal check, just a direct conversation with a real past host about a real stay.
- **Host dashboard** gets a new "Reference checks" section listing every reference request a host is involved in on either side (as the one who asked, or the one being asked), with a status pill and a link into the thread — this is also how a past host *discovers* that someone asked about a former tenant, since there's no notification system yet.
...
## Phase 14.5 — Fixed a blank-space bug on the listing detail page

- A host mistook the listing detail page for having ended partway down, on a lease listing, because of a real layout bug: `.listing-main` (`public/styles.css`) had `flex: 0 1 640px` left over from when the listing header was a row layout (photo left, text right). After an earlier phase switched `.listing-head` to a centered *column* layout, that same flex-basis value started setting the text block's preferred *height* to 640px instead of its width, leaving ~400px of invisible empty space between the header and the rest of the page. Fixed by changing it to `flex: 0 1 auto` (width is already constrained separately by `max-width: 640px`).
- Added a small bouncing "more below" chevron (`.scroll-cue` in `public/styles.css`, rendered in `views/listing.js` right after the listing header) as a standing visual cue that there's more content to scroll to — general insurance against this kind of thing being missed again, not specific to the bug above.

## Phase 15 — "About Kanto" marketing splash page

- **The problem this solves**: the app's homepage (`/`, `views/home.js`) is the functional Browse/search grid — it has no page that pitches the *idea* of Kanto to a first-time visitor (either side) before they're dropped into search results or a signup form.
- **New page**: `/about` (`aboutView` in the new `views/about.js`, wired up in `server.js`). Laid out like a two-column marketing splash — a headline, three value-prop bullets, and two CTAs ("Browse listings" / "List your space") on the left; a stack of three numbered feature callouts on the right, each paired with a small on-brand UI preview.
- The three previews (`miniBrowsePreview`, `miniFlowPreview`, `miniDashboardPreview` in `views/about.js`) are **not screenshots** — they're small live HTML/CSS mockups built from the app's own component styles (`.tile-stay/lease/storage` gradients, `.stat-tile`-style tiles, pill-shaped flow steps). This was a deliberate choice over embedding actual screenshot images: they stay pixel-correct and on-theme automatically (including dark mode) as the real styles evolve, with zero image assets to keep in sync. New CSS for all of this lives in its own block at the end of `public/styles.css` (`.about-*`, `.showcase-*`, `.mini-*`).
- "List your space" links to `/signup?role=host` — `handleSignupGet` in `server.js` now reads a `?role=host` query param and pre-checks the host radio button on the signup form (`views/auth.js` already supported a pre-filled `values.role`, this just wires a query param into it).
...
## Phase 16 — Guest-facing listing page redesign (hero carousel + scroll-snap)

- **The problem this solves**: the listing detail page (`/listings/:id`) looked the same for a browsing guest as it did for the host managing it — a plain header, then stacked info boxes. Based on a resort-website reference the user shared (full-bleed hero photo, overlay title, floating price/search bar), guests now get a page built to *sell* the space, not just list its specs.
- **`views/listing.js` now branches on viewer**: `listingView()` dispatches to `ownerListingView()` (the original layout, byte-for-byte unchanged — verified via curl that the host still gets `<main class="container">`, the old `.listing-head`/`.detail-grid` markup, no `listing-snap` class) or `guestListingView()` (new), based on `user.id === listing.host_id`. A booking-CTA helper (`bookingActionMarkup()`) was pulled out so both views share the exact same needs-application / pending / pay-and-book / login-prompt logic instead of duplicating it.
- **Hero photo carousel** (`heroCarousel()` in `views/listing.js`, controller in the new `public/listing.js`): Kanto has no real photo uploads yet, so the "photos" are 3 CSS-only gradient/pattern slides per listing (two gradient angles + a radial variant), built from the listing's existing per-vertical brand colors, with the vertical's icon watermarked faintly in the center — a deliberate stand-in, not a placeholder that pretends to be a real photo. Auto-advances every 5s; prev/next arrows and dot indicators all work and reset the auto-advance timer on manual use, so it doesn't fight the visitor. Swapped for real photos in one place (`heroCarousel()`) whenever photo upload gets built.
- **Floating price bar** (`.float-bar`) overlaps the bottom of the hero by 34px, resort-website style, showing price, quick specs, and a jump-to-booking CTA at all times.
- **"Page-flip" scroll**: below the hero, the rest of the content (About/Amenities, Availability calendar, the booking/apply box) is split into full-viewport `.snap-section` blocks with `scroll-snap-type: y mandatory` on `<html>` (via a new `htmlClass` passed through `layout()` → `server.js`'s `render()`) and `scroll-snap-stop: always`, so scrolling snaps discretely from section to section rather than free-scrolling — per the user's explicit choice over a fade/continuous-reveal style.
- **Full-bleed hero**: `layout()` gained a `fullBleed` flag that swaps the usual `<main class="container">` for `<main class="main-full-bleed">` (no padding/max-width) so the hero can run edge-to-edge; each section below re-establishes `.container` width itself.
- Fixed one bug found in testing: the carousel's dot indicators were initially positioned at `bottom: 22px`, which put them underneath the floating price bar's opaque overlap and made them invisible — moved to `bottom: 52px` and verified visible in light mode, dark mode, and mobile.
- Verified with Playwright: carousel auto-advance and manual arrow/dot clicks (checked the actual `translateX` transform value), scroll-snap wheel-gesture behavior (clean discrete `scrollY` jumps per section), light/dark mode, desktop and mobile viewports, and that the host/owner view is completely untouched.

## Phase 16.1 — Listing page follow-up fixes

Follow-up requests after seeing Phase 16 live:

- **Mobile hero title was covering too much of the photo.** Added a `@media (max-width: 480px)` block that shrinks the vertical-type pill, title (clamped and capped to 2 lines with `-webkit-line-clamp`), location line, and CTA buttons in `.hero-overlay` — the text stack now covers roughly a quarter of the hero instead of a third.
- **Removed the floating price/CTA tile entirely.** It duplicated the hero's own CTA and (see below) was also the cause of the imprecise snapping. The price now lives inline at the top of the "About this space" card (`.about-price-line`/`.about-price` in `public/styles.css`, `views/listing.js`'s `guestListingView`).
- **Fixed imprecise scroll-snapping ("moves a little bit up or down, not exactly one page").** Two real bugs, not one: (1) every section's height was calculated as `calc(100vh - 65px)`, a hardcoded guess at the sticky header's height — the header actually renders at ~71px, and that few-pixel mismatch compounded section over section. (2) more importantly, nothing told the browser's scroll-snap engine that a sticky header sits on top of the page, so each section's "start" edge was snapping to the literal top of the viewport — hidden behind the header — instead of to the space actually visible below it. Fixed both: `public/listing.js` now measures the header's real height with a `ResizeObserver` and writes it to a `--header-h` CSS variable (kept in sync across font loads/resizes/mobile nav wrapping), which every `calc(100vh - ...)` in `public/styles.css` now reads instead of a hardcoded number; and `html.listing-snap` now sets `scroll-padding-top: var(--header-h)`, the CSS property made for exactly this "content scrolls under a sticky header" situation. Verified with Playwright that each section now lands within a fraction of a pixel of flush against the header, every time. Also removed the hero's `max-height: 760px` cap so it's sized the same "exactly one screen" way as every other section instead of leaving a partial peek of the next one.
- **Hero CTA now goes to the calendar, plus a message-host option.** "Check availability" now jumps to the Availability section (`#availability`, added as an id on that snap-section; falls back to `#book` if a listing has no calendar) instead of straight to the booking form. A second, understated "Message the host" link sits next to it (`.hero-cta-row`/`.hero-cta-secondary`) and goes straight to the inquiry flow — respecting the same signed-out/no-conversation-yet/already-messaged states as the "Have a question?" box further down (`messageHostHref()` in `views/listing.js`).
